    ...
    <div>
      ...
      <div className='p-6'>
        <div className='flex items-center justify-between mb-6'>
          ...
          <button className='px-4 py-1.5 bg-accent hover:bg-accent-hover text-white text-sm font-medium rounded-lg transition-colors'>+ Add Staff</button>
        </div>
        {loading ? (
                ...
                <div className='flex items-start justify-between mb-4'>
                  <div className='flex items-center gap-3'>
                    <div className='w-10 h-10 rounded-full bg-accent/20 flex items-center justify-center'>
                      <span className='text-sm font-bold text-accent'>{member.name.split(' ').map((n: string) => n[0]).join('')}</span>
                    </div>
                    <div>
