...
import { bookings as initialBookings, type Booking, type BookingStatus } from '@/lib/mock-data';
import { formatCurrency } from '@/lib/utils';
import { Search } from 'lucide-react';

const STATUS_OPTIONS: BookingStatus[] = ['confirmed', 'pending', 'seated', 'completed', 'cancelled'];
        ...
        <form onSubmit={handleCreate} className='space-y-4'>
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div>
              ...
              <input required value={form.table} onChange={e => setForm(f => ({ ...f, table: e.target.value }))}
                placeholder='VIP-01' className='w-full px-3 py-2 bg-surface-2 border border-border rounded-lg text-sm text-foreground placeholder:text-foreground-subtle focus:outline-none focus:border-accent/50 transition-colors' />
            </div>
            <div>
              <label className='text-xs font-medium text-foreground-muted mb-1.5 block'>Party Size</label>
              <input required type='number' min='1' max='20' value={form.partySize} onChange={e => setForm(f => ({ ...f, partySize: e.target.value }))}
                placeholder='4' className='w-full px-3 py-2 bg-surface-2 border border-border rounded-lg text-sm text-foreground placeholder:text-foreground-subtle focus:outline-none focus:border-accent/50 transition-colors' />
            </div>
            <div className='col-span-2'>
            ...
            <div className='col-span-2'>
              ...
              <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                placeholder='Birthday, dietary requirements, VIP requests...' rows={2}
